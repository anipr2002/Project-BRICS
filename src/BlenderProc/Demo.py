...
demo_path = "/data/reddy/Final_Presentation/"
...
def manipulate_brick(brick, place):
    ...
    return brick.get_name(), section_mids[place], brick.get_rotation_euler()
...
def render_scene():
    ...
    bproc.writer.write_coco_annotations(os.path.join("/data/reddy/Final_Presentation/", 'coco_data'),
...
for run in range(1):
    ...
    for i, j in enumerate(chosen):
        ...
        location = manipulate_brick(j, random_list[i])
        place_dict[location[0]] = [(round(location[1][0], 3), round(location[1][1], 3), 0), list(location[2])]
...
with open("/data/reddy/Final_Presentation/brick_placement.json", "w") as f:
    json.dump(place_dict, f)
